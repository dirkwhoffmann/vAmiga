...
typedef struct FileSysResource
{
    u32     addr;
    
    struct  Node fsr_Node;              // 0
    u32     fsr_Creator;                // 14
    struct  List fsr_FileSysEntries;    // 18
}
FileSysResource;

typedef struct FileSysEntry
{
    u32     addr;
    
    struct  Node fse_Node;              // 0
    u32     fse_DosType;                // 14
    u32     fse_Version;                // 18
    u32     fse_PatchFlags;             // 22
    u32     fse_Type;                   // 26
    u32     fse_Task;                   // 30
    u32     fse_Lock;                   // 34
    u32     fse_Handler;                // 38
    u32     fse_StackSize;              // 42
    i32     fse_Priority;               // 46
    u32     fse_Startup;                // 50
    u32     fse_SegList;                // 54
    u32     fse_GlobalVec;              // 58
}
FileSysEntry;
