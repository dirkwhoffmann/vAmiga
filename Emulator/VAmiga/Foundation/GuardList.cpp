...
{
    if (guards.isSetAt(target)) throw CoreException(Fault::GUARD_ALREADY_SET, target);
...
{
    if (!guards.guardNr(nr)) throw CoreException(Fault::GUARD_NOT_FOUND, nr);
...
{
    if (!guards.isSet(nr)) throw CoreException(Fault::GUARD_NOT_FOUND, nr);
    guards.remove(nr);
    update();
    emu.main.msgQueue.put(Msg::GUARD_UPDATED);
}
...
{
    if (!guards.isSetAt(target)) throw CoreException(Fault::GUARD_NOT_FOUND, target);
