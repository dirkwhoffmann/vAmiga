...
Memory::checkOption(Opt opt, i64 value)
{
    switch (opt) {
        ...
        case Opt::MEM_CHIP_RAM:
            ...
            if (!isPoweredOff()) {
                throw CoreException(CoreError::OPT_LOCKED);
            }
            if (value != 256 && value != 512 && value != 1024 && value != 2048) {
                throw CoreException(CoreError::OPT_INV_ARG, "256, 512, 1024, 2048");
            }
            return;
        ...
        case Opt::MEM_SLOW_RAM:
            ...
            if ((value % 256) != 0 || value > 1536) {
                throw CoreException(CoreError::OPT_INV_ARG, "0, 256, 512, ..., 1536");
            }
            return;
        ...
        case Opt::MEM_FAST_RAM:
            ...
            if ((value % 64) != 0 || value > 8192) {
                throw CoreException(CoreError::OPT_INV_ARG, "0, 64, 128, ..., 8192");
            }
            return;
        ...
        case Opt::MEM_EXT_START:
            ...
            if (value != 0xE0 && value != 0xF0) {
                throw CoreException(CoreError::OPT_INV_ARG, "E0, F0");
            }
            return;
        ...
        case Opt::MEM_BANKMAP:
            ...
            if (!BankMapEnum::isValid(value)) {
                throw CoreException(CoreError::OPT_INV_ARG, BankMapEnum::keyList());
            }
            return;
        ...
        case Opt::MEM_UNMAPPING_TYPE:
            ...
            if (!UnmappedMemoryEnum::isValid(value)) {
                throw CoreException(CoreError::OPT_INV_ARG, UnmappedMemoryEnum::keyList());
            }
            return;
        ...
        case Opt::MEM_RAM_INIT_PATTERN:
            ...
            if (!RamInitPatternEnum::isValid(value)) {
                throw CoreException(CoreError::OPT_INV_ARG, RamInitPatternEnum::keyList());
            }
            return;
...
Memory::operator << (SerReader &worker)
{
    ...
    if (romSize > KB(512)) throw CoreException(CoreError::SNAP_CORRUPTED);
    if (womSize > KB(256)) throw CoreException(CoreError::SNAP_CORRUPTED);
    if (extSize > KB(512)) throw CoreException(CoreError::SNAP_CORRUPTED);
    if (chipSize > MB(2)) throw CoreException(CoreError::SNAP_CORRUPTED);
    if (slowSize > KB(1792)) throw CoreException(CoreError::SNAP_CORRUPTED);
    if (fastSize > MB(8)) throw CoreException(CoreError::SNAP_CORRUPTED);
...
{    
    ...
    if (!hasRom || FORCE_ROM_MISSING) {
        throw CoreException(CoreError::ROM_MISSING);
    }
    if (!chip || FORCE_CHIP_RAM_MISSING) {
        throw CoreException(CoreError::CHIP_RAM_MISSING);
    }
    if ((hasAros && !ext) || FORCE_AROS_NO_EXTROM) {
        throw CoreException(CoreError::AROS_NO_EXTROM);
    }
    if ((hasAros && ramSize() < MB(1)) || FORCE_AROS_RAM_LIMIT) {
        throw CoreException(CoreError::AROS_RAM_LIMIT);
    }
}
...
{
    // if (amiga.isPoweredOn()) throw CoreException(CoreError::POWERED_ON);
...
{
    if (rom == nullptr) throw CoreException(CoreError::ROM_MISSING);
...
{
    if (wom == nullptr) throw CoreException(CoreError::ROM_MISSING);
...
{
    if (ext == nullptr) throw CoreException(CoreError::ROM_MISSING);
