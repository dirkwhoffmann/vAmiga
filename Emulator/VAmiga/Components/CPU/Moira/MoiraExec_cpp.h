...
Moira::execMovemRgEa(u16 opcode)
{
    ...
    if constexpr (M == 4) {     // -(An)
        ...
        for (int i = 15; i >= 0; i--) {
            ...
            ea -= S;
            if constexpr (C == Core::C68020 && !MIMIC_MUSASHI) writeA(dst, ea);
