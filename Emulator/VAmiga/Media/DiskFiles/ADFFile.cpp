...
ADFFile::fileSize(Diameter diameter, Density density)
{
    ...
    if (diameter != Diameter::INCH_35) throw CoreException(CoreError::DISK_INVALID_DIAMETER);
    
    if (density == Density::DD) return ADFSIZE_35_DD;
    if (density == Density::HD) return ADFSIZE_35_HD;

    throw CoreException(CoreError::DISK_INVALID_DENSITY);
}
...
{
    if (descr.diameter != Diameter::INCH_35) throw CoreException(CoreError::DISK_INVALID_DIAMETER);
    ...
    switch (descr.density) {
        ...
        case Density::DD:
            ...
            switch (descr.cylinders) {
                ...
                default:
                    throw CoreException(CoreError::DISK_INVALID_LAYOUT);
            }
            break;
...
{
    if (drive.disk == nullptr) throw CoreException(CoreError::DISK_MISSING);
    init(*drive.disk);
}
...
ADFFile::init(MutableFileSystem &volume)
{
    switch (volume.numBlocks()) {
        ...
        default:
            throw CoreException(CoreError::FS_WRONG_CAPACITY);
    }
...
ADFFile::formatDisk(FSVolumeType fs, BootBlockId id, string name)
{
    ...
    if (!volume.exportVolume(data.ptr, data.size)) throw CoreException(CoreError::FS_UNKNOWN);
}
...
ADFFile::encodeDisk(FloppyDisk &disk) const
{
    if (disk.getDiameter() != getDiameter()) {
        throw CoreException(CoreError::DISK_INVALID_DIAMETER);
    }
...
{ 
    ...
    if (nr != sectors) {
        ...
        throw CoreException(CoreError::DISK_WRONG_SECTOR_COUNT);
    }
...
ADFFile::decodeSector(u8 *dst, u8 *src)
{
    ...
    if (sector >= numSectors()) {
        ...
        throw CoreException(CoreError::DISK_INVALID_SECTOR_NUMBER);
    }
