...
OSDebugger::checkExecBase(const os::ExecBase &execBase) const
{
    ...
    if (!(IS_EVEN(execBase.addr) && mem.inRam(execBase.addr))) {
        throw CoreException(Fault::OSDB, "ExecBase: Invalid address");
    }

    // Check if ChkBase is the bitwise complement of SysBase
    if (!(execBase.ChkBase == ~execBase.addr)) {
        throw CoreException(Fault::OSDB, "ExecBase: Invalid ChkSum");
    }
    ...
    if (!(checksum == 0xFFFF)) {
        throw CoreException(Fault::OSDB, "ExecBase: Checksum mismatch");
    }
    
    // Check if MaxLocMem complies to the bank map
    if (execBase.MaxLocMem & 0xFF000000) {
        throw CoreException(Fault::OSDB, "ExecBase: MaxLocMem is too large");
    }
    if (execBase.MaxLocMem & 0x3FFFF) {
        throw CoreException(Fault::OSDB, "ExecBase: MaxLocMem is not aligned");
    }
    if (auto bank = execBase.MaxLocMem >> 16) {
        ...
        if (!(src1 == MemSrc::CHIP && src2 != MemSrc::CHIP)) {
            throw CoreException(Fault::OSDB, "ExecBase: MaxLocMem doesn't match bank map");
        }
    }

    // Check if MaxExtMem complies to the bank map
    if (execBase.MaxExtMem & 0xFF000000) {
        throw CoreException(Fault::OSDB, "ExecBase: MaxExtMem is too large");
    }
    if (execBase.MaxExtMem & 0x3FFFF) {
        throw CoreException(Fault::OSDB, "ExecBase: MaxExtMem is not aligned");
    }
    if (auto bank = execBase.MaxExtMem >> 16) {
        ...
        if (!(src1 == MemSrc::SLOW && src2 != MemSrc::SLOW)) {
            throw CoreException(Fault::OSDB, "ExecBase: MaxExtMem doesn't match bank map");
        }
    }
}
