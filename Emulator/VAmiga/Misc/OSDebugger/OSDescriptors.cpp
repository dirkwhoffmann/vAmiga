...
ProgramUnitDescriptor::init(const u8 *buf, isize len)
{
    ...
    auto read = [&]() {
        ...
        if (offset + 4 > len) throw CoreException(Fault::HUNK_CORRUPTED);
    ...
    // Check magic cookie
    if (cookie != HUNK_HEADER) throw CoreException(Fault::HUNK_BAD_COOKIE);
    ...
    if (numHunks == 0) throw CoreException(Fault::HUNK_NO_SECTIONS);
    auto first = isize(read());
    if (first != 0) throw CoreException(Fault::HUNK_BAD_HEADER);
    auto last = isize(read());
    if (last != numHunks - 1) throw CoreException(Fault::HUNK_BAD_HEADER);
    ...
    for (isize h = 0; h < numHunks; ) {
        ...
        switch (type) {
            ...
            default:
                ...
                throw CoreException(Fault::HUNK_UNSUPPORTED, HunkTypeEnum::key(type));
        }
    }
}
