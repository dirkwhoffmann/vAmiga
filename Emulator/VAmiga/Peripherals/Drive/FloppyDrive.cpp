    ...
    switch (opt) {
        ...
        case Opt::DRIVE_TYPE:
            ...
            if (!FloppyDriveTypeEnum::isValid(value)) {
                throw CoreException(Fault::OPT_INV_ARG, FloppyDriveTypeEnum::keyList());
            }
        ...
        case Opt::DRIVE_MECHANICS:
            ...
            if (!DriveMechanicsEnum::isValid(value)) {
                throw CoreException(Fault::OPT_INV_ARG, DriveMechanicsEnum::keyList());
            }
            return;
...
FloppyDrive::insertDisk(std::unique_ptr<FloppyDisk> disk, Cycle delay)
{
    ...
    if (!isInsertable(*disk)) throw CoreException(Fault::DISK_INCOMPATIBLE);
...
FloppyDrive::catchFile(const std::filesystem::path &path)
{
    ...
    if (file == nullptr) throw CoreException(Fault::FILE_NOT_FOUND);
    ...
    if (!offset) throw CoreException(Fault::HUNK_CORRUPTED);
