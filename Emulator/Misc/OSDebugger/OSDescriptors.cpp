...
ProgramUnitDescriptor::init(const u8 *buf, isize len)
{
    ...
    auto read = [&]() {
        ...
        if (offset + 4 > len) throw VAException(VAError::HUNK_CORRUPTED);
    ...
    // Check magic cookie
    if (cookie != HUNK_HEADER) throw VAException(VAError::HUNK_BAD_COOKIE);
    ...
    if (numHunks == 0) throw VAException(VAError::HUNK_NO_SECTIONS);
    auto first = isize(read());
    if (first != 0) throw VAException(VAError::HUNK_BAD_HEADER);
    auto last = isize(read());
    if (last != numHunks - 1) throw VAException(VAError::HUNK_BAD_HEADER);
    ...
    for (isize h = 0; h < numHunks; ) {
        ...
        switch (type) {
            ...
            case HUNK_RELOC32:
                ...
                for (auto count = read(); count; count = read()) {
                    ...
                    if (count > KB(64)) {
                        ...
                        throw VAException(VAError::HUNK_CORRUPTED);
                    }
            ...
            default:
                ...
                throw VAException(VAError::HUNK_UNSUPPORTED, HunkTypeEnum::key(type));
        }
    }
}
