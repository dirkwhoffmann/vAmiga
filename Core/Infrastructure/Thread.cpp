...
Thread::execute()
{
    ...
    if (std::abs(missing) <= 5) {
        ...
        if (missing > 0) {
            logme(RUN_DEBUG, "Emulation is way too slow (%ld frames behind)\n", missing);
        } else {
            logme(RUN_DEBUG, "Emulation is way too fast (%ld time slices ahead)\n", -missing);
        }
