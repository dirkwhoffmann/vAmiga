...
const u8 RomFile::kickRomHeaders[15][7] = {

    // AROS Kickstart replacement
    { 0x11, 0x14, 0x4E, 0xF9, 0x00, 0xF8, 0x00 }, // Aros Rom and Aros extended Rom
    { 0x11, 0x11, 0x4E, 0xF9, 0x00, 0xF8, 0x00 }, // Aros Rom only
    { 0x4E, 0x71, 0x4E, 0xF9, 0x00, 0xF8, 0x00 }, // Extended Rom only
    ...
    // CDTV Extension ROMs
    { 0x11, 0x11, 0x4E, 0xF9, 0x00, 0xF0, 0x00 },
    { 0x11, 0x14, 0x4E, 0xF9, 0x00, 0xF0, 0x00 },
...
RomFile::decrypt()
{
    ...
    if (romKey.empty()) throw AppError(AppError::MISSING_ROM_KEY);
    ...
    if (!isCompatible(data.ptr, data.size)) {
        throw AppError(AppError::INVALID_ROM_KEY);
    }
}
