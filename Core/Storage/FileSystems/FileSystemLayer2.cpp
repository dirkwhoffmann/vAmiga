...
FileSystem::cd(const FSName &name)
{
    ...
    throw FSError(FSFault::FS_NOT_FOUND, name.cpp_str());
}
...
FileSystem::cd(const string &path)
{
    ...
    throw FSError(FSFault::FS_NOT_FOUND, path);
}
...
{
    if (auto *it = seekPtr(&root, name); it) return *it;
    throw FSError(FSFault::FS_NOT_FOUND, name.string());
}
...
{
    if (auto *it = seekPtr(&root, name); it) return *it;
    throw FSError(FSFault::FS_NOT_FOUND, name);
}
