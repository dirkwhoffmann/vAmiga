...
FSImporter::importVolume(const u8 *src, isize size)
{
    ...
    if (size % traits.bsize != 0) throw FSError(FSFault::FS_WRONG_BSIZE);

    // Only proceed if the source buffer contains the right amount of data
    if (traits.bytes != size) throw FSError(FSFault::FS_WRONG_CAPACITY);

    // Only proceed if all partitions contain a valid file system
    if (traits.dos == FSFormat::NODOS) throw FSError(FSFault::FS_UNSUPPORTED);
