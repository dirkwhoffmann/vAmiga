...
FSAllocator::requiredBlocks(isize fileSize) const noexcept
{
    ...
    logme(FS_DEBUG, "Required file header blocks : %d\n",  1);
    logme(FS_DEBUG, "       Required data blocks : %ld\n", numDataBlocks);
    logme(FS_DEBUG, "  Required file list blocks : %ld\n", numFileListBlocks);
...
FSAllocator::allocate()
{
    ...
    while (!fs.isEmpty(i)) {
        ...
        if ((i = (i + 1) % numBlocks) == ap) {
            ...
            logme(FS_DEBUG, "No more free blocks\n");
                                ...
                                std::vector<BlockNr> &dataBlocks)
{
    ...
    */
    ...
    logme(FS_DEBUG, "                   Data bytes : %ld\n", bytes);
    logme(FS_DEBUG, "         Required data blocks : %ld\n", numDataBlocks);
    logme(FS_DEBUG, "         Required list blocks : %ld\n", numListBlocks);
    logme(FS_DEBUG, "         References per block : %ld\n", refsPerBlock);
    logme(FS_DEBUG, "   References in header block : %ld\n", refsInHeaderBlock);
    logme(FS_DEBUG, "    References in list blocks : %ld\n", refsInListBlocks);
    logme(FS_DEBUG, "References in last list block : %ld\n", refsInLastListBlock);
...
FSAllocator::locateAllocationBit(BlockNr nr, isize *byte, isize *bit) const noexcept
{
    ...
    if (bmNr >= (isize)bmBlocks.size()) {
        logme(FS_DEBUG, "Bitmap block index %ld for block %ld is out of range \n", bmNr, nr);
        return nullptr;
    }
    ...
    if (!bm.is(FSBlockType::BITMAP)) {
        logme(FS_DEBUG, "Failed to lookup allocation bit for block %ld (%ld)\n", nr, bmNr);
        return nullptr;
    }
...
FSAllocator::numUnallocated() const noexcept
{
    ...
    if CONSTEXPR (debug::FS_DEBUG != -1) {
        ...
        logme(FS_DEBUG, "Unallocated blocks: Fast code: %ld Slow code: %ld\n", result, count);
        assert(count == result);
    }
