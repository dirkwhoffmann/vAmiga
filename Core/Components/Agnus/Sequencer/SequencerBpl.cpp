...
Sequencer::computeBplEventTable(const SigRecorder &sr)
{
    debugmsg(SEQ_DEBUG, "computeBplEvents\n");
    ...
    if (state != ddfInitial) {
        ...
        debugmsg(SEQ_DEBUG, "Recompute table in next line\n");
...
Sequencer::computeBplEventsFast(const SigRecorder &sr, DDFState &state)
{
    ...
    debugmsg(SEQ_DEBUG, "Fast path (no bitplane DMA in this line)\n");
