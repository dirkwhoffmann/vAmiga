...
{
    logmsg(LOG_BLTREG, "pokeBLTCON0(%X)\n", value);
...
{
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCON0 written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTCON0L(%X)\n", value);
...
{
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCON0L written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTCON1(%X)\n", value);
...
{
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCON1 written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTAPTH(%X)\n", value);

    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTAPTH written while Blitter is running\n");
    }

    bltapt = REPLACE_HI_WORD(bltapt, value);

    if (bltapt & ~agnus.ptrMask) {
        logmsg(LOG_BLT_REG_GUARD, "BLTAPT out of range: %x\n", bltapt);
    }
}
...
{
    logmsg(LOG_BLTREG, "pokeBLTAPTL(%X)\n", value);
    
    if(running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTAPTL written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTBPTH(%X)\n", value);
    
    if(running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTBPTH written while Blitter is running\n");
    }
    
    bltbpt = REPLACE_HI_WORD(bltbpt, value);
    
    if (bltbpt & ~agnus.ptrMask) {
        logmsg(LOG_BLT_REG_GUARD, "BLTBPT out of range: %x\n", bltbpt);
    }
}
...
{
    logmsg(LOG_BLTREG, "pokeBLTBPTL(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTBPTL written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTCPTH(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCPTH written while Blitter is running\n");
    }
    
    bltcpt = REPLACE_HI_WORD(bltcpt, value);
    
    if (bltcpt & ~agnus.ptrMask) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCPT out of range: %x\n", bltcpt);
    }
}
...
{
    logmsg(LOG_BLTREG, "pokeBLTCPTL(%X)\n", value);
    
    if(running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCPTL written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTDPTH(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTDPTH written while Blitter is running\n");
    }

    bltdpt = REPLACE_HI_WORD(bltdpt, value);
    
    if (bltdpt & ~agnus.ptrMask) {
        logmsg(LOG_BLT_REG_GUARD, "BLTDPT out of range: %x\n", bltdpt);
    }
}
...
{
    logmsg(LOG_BLTREG, "pokeBLTDPTL(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTDPTL written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTAFWM(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTAFWM written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTALWM(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTALWM written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "setBLTSIZE(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTSIZE written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTSIZV(%X)\n", value);
...
{
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTSIZV written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTSIZH(%X)\n", value);

    // ECS only register
    if (agnus.isOCS()) return;

    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTSIZH written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTAMOD(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTAMOD written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTBMOD(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTBMOD written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTCMOD(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCMOD written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTDMOD(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTDMOD written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTADAT(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTADAT written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTBDAT(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTBDAT written while Blitter is running\n");
    }
...
{
    logmsg(LOG_BLTREG, "pokeBLTCDAT(%X)\n", value);
    
    if (running) {
        logmsg(LOG_BLT_REG_GUARD, "BLTCDAT written while Blitter is running\n");
    }
...
Blitter::pokeDMACON(u16 oldValue, u16 newValue)
{
    ...
    if (running && oldBltDma && !newBltDma) {
        logmsg(LOG_BLT_REG_GUARD, "Blitter DMA off while Blitter is running\n");
    }
    if (running && agnus.bltpri(oldValue) != agnus.bltpri(newValue)) {
        logmsg(LOG_BLT_REG_GUARD, "BLTPRI changed while Blitter is running\n");
    }
}
