...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCON0(%X)\n", value);
...
{
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTCON0 written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCON0L(%X)\n", value);
...
{
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTCON0L written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCON1(%X)\n", value);
...
{
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTCON1 written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTAPTH(%X)\n", value);

    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTAPTH written while Blitter is running\n");
    }

    bltapt = REPLACE_HI_WORD(bltapt, value);

    if (bltapt & ~agnus.ptrMask) {
        debugmsg(BLT_REG_GUARD, "BLTAPT out of range: %x\n", bltapt);
    }
}
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTAPTL(%X)\n", value);
    
    if(running) {
        debugmsg(BLT_REG_GUARD, "BLTAPTL written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTBPTH(%X)\n", value);
    
    if(running) {
        debugmsg(BLT_REG_GUARD, "BLTBPTH written while Blitter is running\n");
    }
    
    bltbpt = REPLACE_HI_WORD(bltbpt, value);
    
    if (bltbpt & ~agnus.ptrMask) {
        debugmsg(BLT_REG_GUARD, "BLTBPT out of range: %x\n", bltbpt);
    }
}
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTBPTL(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTBPTL written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCPTH(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTCPTH written while Blitter is running\n");
    }
    
    bltcpt = REPLACE_HI_WORD(bltcpt, value);
    
    if (bltcpt & ~agnus.ptrMask) {
        debugmsg(BLT_REG_GUARD, "BLTCPT out of range: %x\n", bltcpt);
    }
}
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCPTL(%X)\n", value);
    
    if(running) {
        debugmsg(BLT_REG_GUARD, "BLTCPTL written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTDPTH(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTDPTH written while Blitter is running\n");
    }

    bltdpt = REPLACE_HI_WORD(bltdpt, value);
    
    if (bltdpt & ~agnus.ptrMask) {
        debugmsg(BLT_REG_GUARD, "BLTDPT out of range: %x\n", bltdpt);
    }
}
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTDPTL(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTDPTL written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTAFWM(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTAFWM written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTALWM(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTALWM written while Blitter is running\n");
    }
...
{
    debugmsg(BLTTIM_DEBUG, "(%ld,%ld) BLTSIZE(%x)\n", agnus.pos.v, agnus.pos.h, value);
    debugmsg(BLTREG_DEBUG, "pokeBLTSIZE(%X)\n", value);
...
{
    debugmsg(BLTREG_DEBUG, "setBLTSIZE(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTSIZE written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTSIZV(%X)\n", value);
...
{
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTSIZV written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTSIZH(%X)\n", value);

    // ECS only register
    if (agnus.isOCS()) return;

    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTSIZH written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTAMOD(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTAMOD written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTBMOD(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTBMOD written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCMOD(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTCMOD written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTDMOD(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTDMOD written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTADAT(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTADAT written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTBDAT(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTBDAT written while Blitter is running\n");
    }
...
{
    debugmsg(BLTREG_DEBUG, "pokeBLTCDAT(%X)\n", value);
    
    if (running) {
        debugmsg(BLT_REG_GUARD, "BLTCDAT written while Blitter is running\n");
    }
...
Blitter::pokeDMACON(u16 oldValue, u16 newValue)
{
    ...
    if (running && oldBltDma && !newBltDma) {
        debugmsg(BLT_REG_GUARD, "Blitter DMA off while Blitter is running\n");
    }
    if (running && agnus.bltpri(oldValue) != agnus.bltpri(newValue)) {
        debugmsg(BLT_REG_GUARD, "BLTPRI changed while Blitter is running\n");
    }
}
