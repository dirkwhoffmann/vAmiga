...
TOD::getCounterHi(Cycle timeStamp) const
{
    ...
    logtrace(TOD_DEBUG, "getCounterHi: %02x\n", result);
    return result;
}
...
TOD::getCounterMid(Cycle timeStamp) const
{
    ...
    logtrace(TOD_DEBUG, "getCounterMid: %02x\n", result);
    return result;
}
...
TOD::getCounterLo(Cycle timeStamp) const
{
    ...
    logtrace(TOD_DEBUG, "getCounterLo: %02x\n", result);
    return result;
}
...
{
    logtrace(TOD_DEBUG, "getAlarmHi: %02x\n", alarm.hi);
    return alarm.hi;
}
...
{
    logtrace(TOD_DEBUG, "getAlarmMid: %02x\n", alarm.mid);
    return alarm.mid;
}
...
{
    logtrace(TOD_DEBUG, "getAlarmLo: %02x\n", alarm.lo);
    return alarm.lo;
}
...
{
    logtrace(TOD_DEBUG, "setCounterHi(%x)\n", value);
    tod.hi = value;
...
{
    logtrace(TOD_DEBUG, "setCounterMid(%x)\n", value);
    tod.mid = value;
...
{
    logtrace(TOD_DEBUG, "setCounterLo(%x)\n", value);
    tod.lo = value;
...
{
    logtrace(TOD_DEBUG, "setAlarmHi(%x)\n", value);
    alarm.hi = value;
...
{
    logtrace(TOD_DEBUG, "setAlarmMid(%x)\n", value);
    alarm.mid = value;
...
{
    logtrace(TOD_DEBUG, "setAlarmLo(%x)\n", value);
    alarm.lo = value;
...
TOD::increment()
{
    ...
    if (tod.value == alarm.value) {
        logtrace(TOD_DEBUG, "TOD bug hits: %x:%x:%x (%d,%d)\n",
...
TOD::checkIrq()
{
    if (!matching && tod.value == alarm.value) {
        ...
        logtrace(TOD_DEBUG, "TOD IRQ (%02x:%02x:%02x)\n", tod.hi, tod.mid, tod.lo);
        cia.todInterrupt();
    }
